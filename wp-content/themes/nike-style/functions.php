...
// ------------------------------------------------------------------
// FORMATO DE PRECIOS EN CLP (Peso chileno) — sin warnings de PHP
// ------------------------------------------------------------------
...
// Formato del símbolo de moneda: "CLP" al lado del monto
...
// ------------------------------------------------------------------
// Navegación de categorías en el header
// ------------------------------------------------------------------
...
// CSS para barra superior (solo base, el resto va en style.css)
...
// ------------------------------------------------------------------
// HACER QUE LA PORTADA SEA UNA TIENDA, NO UN BLOG
// ------------------------------------------------------------------

// 1) Traducir "Add to cart" a español para productos
add_filter( 'woocommerce_product_add_to_cart_text', 'nike_style_translate_add_to_cart', 10, 2 );
function nike_style_translate_add_to_cart( $text, $product ) {
    if ( $product->is_type( 'variable' ) ) {
        return __( 'Seleccionar opciones', 'nike-style' );
    }
...
// 3) Ocultar la barra lateral (sidebar) de widgets de blog en la portada
function nike_style_remove_home_sidebar() {
    if ( is_front_page() ) {
        remove_action( 'storefront_sidebar', 'storefront_get_sidebar', 10 );
    }
}
add_action( 'wp', 'nike_style_remove_home_sidebar' );

// 4) Hacer la portada a ancho completo (full width)
function nike_style_home_full_width( $classes ) {
    if ( is_front_page() ) {
...
add_filter( 'body_class', 'nike_style_home_full_width' );

// 5) Ocultar el título "Inicio" de la portada (evitar parecer blog)
...
// 6) En la portada, mostrar la tienda (novedades) en lugar de solo contenido vacío
